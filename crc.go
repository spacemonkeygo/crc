...
With modifications and transliteration to Go, 2014 Space Monkey, Inc.
...
func crc32_little(crc uint32, data []uint8) uint32 {
	c := ^crc
	buf := &data[0]
	buflen := len(data)

	for buflen > 0 && (uintptr(unsafe.Pointer(buf))&3 > 0) {
		c = crc_table[0][(c^uint32(*buf))&0xff] ^ (c >> 8)
		buf = (*uint8)(unsafe.Pointer(uintptr(unsafe.Pointer(buf)) + 1))
		buflen--
	}

	buf4 := (*uint32)(unsafe.Pointer(buf))
	for buflen >= 32 {
		c ^= *buf4
		buf4 = (*uint32)(unsafe.Pointer(uintptr(unsafe.Pointer(buf4)) + 4))
...
func crc32_big(crc uint32, data []uint8) uint32 {
	...
	for buflen > 0 && (uintptr(unsafe.Pointer(buf))&3 > 0) {
		c = crc_table[4][(c>>24)^uint32(*buf)] ^ (c << 8)
		buf = (*uint8)(unsafe.Pointer(uintptr(unsafe.Pointer(buf)) + 1))
		buflen--
	}
